...
Both gcd-in and gcd-out are auto-derived from the selection CSV path:
  CSV:    .../GeometryFiles/340StringMC/102_string.csv
  gcd-in: paths.GCD["340StringMC"]
  Output: .../Metadata/GCD/340StringMC/102_string.i3.gz

Usage example:
  python3 trim_GCD.py \
    --selection /project/def-nahee/kbas/Graphnet-Applications/Metadata/GeometryFiles/340StringMC/102_string.csv
...
def derive_gcd_in(selection: Path) -> Path:
    """Look up GCD input from paths.GCD using the CSV's parent folder name."""
    mc_name = selection.parent.name
    if mc_name not in paths.GCD:
        raise KeyError(f"No GCD entry in paths.GCD for '{mc_name}'. Available: {list(paths.GCD)}")
    return Path(paths.GCD[mc_name])


def derive_gcd_out(selection: Path) -> Path:
    """Auto-derive output path from CSV: BASE_GCD_OUT/<parent_folder>/<stem>.i3.gz"""
    return BASE_GCD_OUT / selection.parent.name / (selection.stem + ".i3.gz")
...
def main() -> int:
    ...
    gcd_in = Path(args.gcd_in).resolve() if args.gcd_in is not None else derive_gcd_in(selection)
    gcd_out = Path(args.gcd_out).resolve() if args.gcd_out is not None else derive_gcd_out(selection)
