"""
Submit v3 PMT response jobs to SLURM — single node, parallel execution.

Usage:
    python3 submit_pmt_response.py --mc-name STRING340MC --geometry full_geometry --flavor Muon --with-first-3-layers
    python3 submit_pmt_response.py --mc-name STRING340MC --geometry full_geometry --flavor all --with-first-3-layers --dry-run
...
WORKER_SH    = Path("/home/kbas/SlurmScripts/DataPreperation/submit_pmt_response__pone_offline_version_3.sh")
SCRATCH_BASE = "/home/kbas/scratch"
STRING340_V3_BASE = f"{SCRATCH_BASE}/String340MC_pone_offline_version3"
...
def submit_one(*, mc: str, mc_folder: str, geometry: str, geo_folder: str,
    ...
    if not with_first_3_layers:
        raise ValueError("PONE offline v3 PMT response currently supports only --with-first-3-layers")
    pattern = FORMAT_PATTERN.get(fmt, f"*.i3.{fmt}")
    ...
    if mc == "STRING340MC":
        outdir = f"{STRING340_V3_BASE}/{flavor}_PMT_Response"
        logdir = f"{STRING340_V3_BASE}/Logs/{flavor}_pmt_response"
    else:
        outdir = f"{SCRATCH_BASE}/{mc_folder}/{geo_folder}/{flavor}_PMT_Response_pone_offline_version_3"
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Submit PONE offline v3 PMT response jobs (single node, parallel)")
